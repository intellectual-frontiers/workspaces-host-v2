...
let
  ...
  # mgit.json or sensitivectl's config - home-manager only creates it once
  # (mode 0600, which libpq itself requires or it silently ignores the
  # file) and never touches an existing one again. This stub has no real
  # secrets, just the format documentation the original repo's own
  # checked-in template also shipped as commented-out examples.
...
in
{
  # .psqlrc is plain psql client config - no secrets - so it's fully
  # managed here like any other dotfile in this repo. Ported from the
  # original repo's dot_psqlrc.tmpl (minus its chezmoi templating header).
